...
| Method | Route | Result |
|--------|-------|--------|
| `POST` | `/Homes` | Create a home; `player` in the JSON body is an existing player's username. |
...
The API resolves the username and stores `player` as the player's identifier, never their username or Minecraft UUID. It generates the home's random GUID (`id`) and UTC `createdDT`; `updatedDT` is null until the first patch and is generated by the API thereafter. Creation and patch request contracts contain no writable timestamps, and clients cannot supply or replace the home identifier.

POST and PATCH return the resulting home within `content`. Single GET responses use that identical contract; collection responses contain `content.homes` and `content.count`. Home properties are `id`, `createdDT`, `updatedDT`, `name`, `player`, and `location`.
